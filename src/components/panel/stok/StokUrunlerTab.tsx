...
type StokUrunlerTabProps = {
  initialOpenNew?: boolean;
  onOpenNewConsumed?: () => void;
};

export function StokUrunlerTab(props: StokUrunlerTabProps) {
  const { initialOpenNew, onOpenNewConsumed } = props;
  ...
  useEffect(() => {
    if (initialOpenNew && categories.length > 0 && !openedNewRef.current) {
  ...
  }, [initialOpenNew, categories.length]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap gap-3 items-center">
        <input type="search" placeholder="Kod veya ad..." value={q} onChange={(e) => setQ(e.target.value)} className="panel-input w-52 max-w-full" />
        <select value={categoryId} onChange={(e) => setCategoryId(e.target.value)} className="panel-select w-44">
                  ...
                  <td className="text-right">
                    {!p.archived && (
                      <span className="flex gap-2 justify-end">
                        ...
                        <button type="button" onClick={() => setArchiveTarget(p)} className="text-amber-600 hover:text-amber-700 text-sm font-medium">
                          Arşivle
                        </button>
                      </span>
                    )}
        ...
        <div className="panel-modal-overlay" onClick={() => setArchiveTarget(null)}>
          <div className="panel-modal max-w-sm" onClick={(e) => e.stopPropagation()}>
            <div className="panel-modal-body">
              <p className="text-slate-600 mb-4">"{archiveTarget.name}" ürününü arşivlemek istediğinize emin misiniz?</p>
